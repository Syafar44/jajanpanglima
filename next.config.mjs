import withPWA from "next-pwa";

/** @type {import('next').NextConfig} */
const nextConfig = {
  // Properti lainnya untuk konfigurasi Next.js
  // Misalnya:
  // reactStrictMode: true, // Aktifkan kembali jika diperlukan di masa mendatang
};

// Menggabungkan konfigurasi PWA
const pwaConfig = {
  dest: "public", // Output untuk service worker
  disable: process.env.NODE_ENV === "development", // Nonaktifkan PWA di mode development
  register: true, // Register service worker otomatis
  skipWaiting: true, // Mengganti service worker lama tanpa menunggu
};

// Ekspor konfigurasi dengan PWA
export default withPWA(nextConfig, pwaConfig);
